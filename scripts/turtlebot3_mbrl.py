...
if __name__ == '__main__':
    ...
    for itr in range(n_iter):
        ...
        for train_step in tqdm(range(num_agent_train_steps_per_iter)):
    ...
    rospy.logdebug("------Init ENV")
     # 
    '''
    #TODO write RL trainer similar to modelfree.Step:

    make training env
    create objects: MBagent,
    run base policy to collect D

    run training loop:
        choose action using MPC
        
        perform a step in the enviroment.
        add (s,a) to the replay buffer.
    '''
    score= 0
    n_iter = 20 # number of Dagger iterations
    ...
    ##in some kind of loop that indicate start of a episode
    path : PathDict = sample_trajectory(env, agent.actor,max_path_length)

    ###in somekind of loop, like episode step
    action = agent.actor.get_action(observation)
    next_ob, reward, done = env.step(action)
    agent.add_to_replay_buffer (action,add_sl_noise=True)# T: shoud add the whole paths instead of action
    ...
    for itr in range (n_iter):
        print("\n\n********** Iteration %i ************"%itr)
        if itr == 0:
                use_batchsize = 2000 #(random) steps collected on 1st iteration (put into replay buffer)

        use_batchsize =  8000 ##steps collected per train iteration (put into replay buffer)
